...
class ClassroomCreateBloc
  ...
  ClassroomCreateBloc(this._usecaseCreateClassroom) : super(_initialState) {
    ...
    on<StartTimeChanged>((event, emit) => emit(state.copyWith(
          startTime: event.startTime,
        )));
    on<EndTimeChanged>((event, emit) => emit(state.copyWith(
    ...
    on<SchoolingChanged>((event, emit) => emit(state.copyWith(
          schooling: event.schooling,
        )));
    on<ComplementaryActivityChanged>((event, emit) => emit(state.copyWith(
          complementaryActivity: event.complementaryActivity,
        )));
    on<AeeChanged>((event, emit) => emit(state.copyWith(
          aee: event.aee,
        )));
    on<MoreEducationParticipatorChanged>((event, emit) => emit(state.copyWith(
          moreEducationParticipator: event.moreEducationParticipator,
        )));
    on<AeeBrailleChanged>((event, emit) => emit(state.copyWith(
          aeeBraille: event.aeeBraille,
        )));
    on<AeeOpticalNonOpticalChanged>((event, emit) => emit(state.copyWith(
          aeeOpticalNonOptical: event.aeeOpticalNonOptical,
        )));
    on<AeeMobilityTechniquesChanged>((event, emit) => emit(state.copyWith(
          aeeMobilityTechniques: event.aeeMobilityTechniques,
        )));
    on<AeeCognitiveFunctionsChanged>((event, emit) => emit(state.copyWith(
          aeeCognitiveFunctions: event.aeeCognitiveFunctions,
        )));
    on<AeeLibrasChanged>((event, emit) => emit(state.copyWith(
          aeeLibras: event.aeeLibras,
        )));
    on<AeeCaaChanged>((event, emit) => emit(state.copyWith(
          aeeCaa: event.aeeCaa,
        )));
    on<AeeCurriculumEnrichmentChanged>((event, emit) => emit(state.copyWith(
          aeeCurriculumEnrichment: event.aeeCurriculumEnrichment,
        )));
    on<AeeAccessibleTeachingChanged>((event, emit) => emit(state.copyWith(
          aeeAccessibleTeaching: event.aeeAccessibleTeaching,
        )));
    on<AeePortugueseChanged>((event, emit) => emit(state.copyWith(
          aeePortuguese: event.aeePortuguese,
        )));
    on<AeeSorobanChanged>((event, emit) => emit(state.copyWith(
          aeeSoroban: event.aeeSoroban,
        )));
    on<AeeAutonomousLifeChanged>((event, emit) => emit(state.copyWith(
